...
public static void main(String[] args) throws NumberFormatException, IOException {
	...
	Statement stmt = con.createStatement();) {
		...
		case 1:
			...
			int a = 0; 
			System.out.format("Cust. No \tName     \tPhone No \tCity\n");
			while(rs.next()) {
		...
		case 5:
			System.out.println("Enter Cust No"); 
			cust_no = br.readLine();
			String queryAccDetails= "SELECT A.ACCOUNT_NO, A.TYPE, A.BALANCE, B.BRANCH_CODE,B.BRANCH_NAME,B.BRANCH_CITY FROM ACCOUNT A, BRANCH B, CUSTOMER C, DEPOSITOR D WHERE D.ACCOUNT_NO = A.ACCOUNT_NO AND A.BRANCH_CODE = B.BRANCH_CODE AND C.CUST_NO = D.CUST_NO AND C.CUST_NO = '"+cust_no+"'";
			rs = stmt.executeQuery(queryAccDetails);
			if (!rs.isBeforeFirst()) { 
		        System.out.println("No records found for Customer Number: " + cust_no);
		    } else {
		...
		case 8:
			...
			if (rs.next()) {
				...
				if (amt <= bal) {
					...
					int nP = stmt.executeUpdate(querySubMoney); 
					System.out.println(nP + " Balance Updated");
				} else {
		...
		case 9:
			System.out.println("COME VISIT US SOON!!!"); 
			stmt.close();
			con.close(); 
			System.exit(0);
			break;
